        ...
        <div className="container mx-auto px-4 md:px-0">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-gray-600 mt-1">View the details of this sustainable fashion exchange</p>
            </div>
          ...
          <div className={`mb-8 p-4 rounded-lg flex items-center justify-between ${
            trade.requestedItem.status === 'pending' ? 'bg-yellow-50 border border-yellow-200' :
            trade.requestedItem.status === 'accepted' ? 'bg-green-50 border border-green-200' :
            'bg-red-50 border border-red-200'
          }`}>
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                trade.requestedItem.status === 'pending' ? 'bg-yellow-100' :
                trade.requestedItem.status === 'accepted' ? 'bg-green-100' :
                'bg-red-100'
              }`}>
                <IoSwapHorizontalOutline className={`text-xl ${
                  trade.requestedItem.status === 'pending' ? 'text-yellow-700' :
                  trade.requestedItem.status === 'accepted' ? 'text-green-700' :
                  'text-red-700'
                }`} />
              </div>
              <div>
                ...
                <p className={`text-sm ${
                  trade.requestedItem.status === 'pending' ? 'text-yellow-800' :
                  trade.requestedItem.status === 'accepted' ? 'text-green-800' :
                  'text-red-800'
                }`}>
                  {trade.requestedItem.status === 'pending' ? 'Waiting for response' :
                   trade.requestedItem.status === 'accepted' ? 'Trade accepted' :
                   'Trade declined'}
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 relative">
            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              ...
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-3">{trade.size} / {trade.brand}</p>
              ...
              <div className="p-4 border-t bg-gray-50">
                <h6 className="text-sm font-medium text-gray-500 mb-3">Item Owner</h6>
                <div className="flex items-center gap-3">
            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              ...
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-3">{trade.requestedItem.size} / {trade.requestedItem.brand}</p>
                <h5 className="font-medium text-lg mb-1">{trade.requestedItem.name}</h5>
                <p className="text-sm text-gray-700">{trade.requestedItem.description}</p>
              </div>
              
              {/* Trader Profile */}
              <div className="p-4 border-t bg-gray-50">
                <h6 className="text-sm font-medium text-gray-500 mb-3">Trader Profile</h6>
                <div className="flex items-center gap-3">
                  <div className="relative w-12 h-12">
                    <Image
                  ...
                  <div>
                    <p className="font-medium">{trade.requestedItem.owner.name}</p>
                    <div className="flex items-center gap-4 mt-1 text-sm text-gray-600">
                      <div className="flex items-center gap-1">
                        <IoLocationOutline />
